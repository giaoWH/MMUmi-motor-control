...
{	
    ...
    // 【修改点 1】DataLength 必须使用 STM32 HAL 库提供的宏定义，绝对不能直接赋数字
    switch(len)
    {
    ...
    // 【修改点 2】根据你初始化的格式（经典CAN还是CAN-FD）动态匹配，防止总线报错
...
{	
	...
	while(HAL_FDCAN_GetRxMessage(hfdcan,FDCAN_RX_FIFO0, &pRxHeader, buf)==HAL_OK)
	{
